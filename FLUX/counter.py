...
class TimeTracker:
    ...
    def __init__(self):
        # 使用嵌套字典存储时间，结构为 times[step][layer][category]
        self.times = {}

    def log_time(self, start_time, category, step, layer):
        # 记录时间
        elapsed_time = time.time() - start_time
        if step not in self.times:
            self.times[step] = {}
        if layer not in self.times[step]:
            self.times[step][layer] = {}
        if category not in self.times[step][layer]:
            self.times[step][layer][category] = 0
        self.times[step][layer][category] += elapsed_time

    def show_layer(self, batch_size):
        # 显示所有层的平均时间总和，每个样本的平均时间
        layer_totals = {}
        for step in self.times:
            for layer in self.times[step]:
                if layer not in layer_totals:
                    layer_totals[layer] = 0
                for category in self.times[step][layer]:
                    layer_totals[layer] += self.times[step][layer][category]

        data = [(layer, total / batch_size) for layer, total in layer_totals.items()]
        print("Average time per sample across all steps and categories per layer:")
        print(tabulate(data, headers=['Layer', 'Avg Time (seconds per sample)'], tablefmt='grid'))

    def show_step(self, batch_size):
        # 显示所有步骤的平均时间总和，每个样本的平均时间
        step_totals = {}
        for step in self.times:
            for layer in self.times[step]:
                if step not in step_totals:
                    step_totals[step] = 0
                for category in self.times[step][layer]:
                    step_totals[step] += self.times[step][layer][category]

        data = [(step, total / batch_size) for step, total in step_totals.items()]
        print("Average time per sample across all layers and categories per step:")
        print(tabulate(data, headers=['Step', 'Avg Time (seconds per sample)'], tablefmt='grid'))

    def show_category(self, batch_size):
        # 显示所有类别的平均时间总和，每个样本的平均时间
        category_totals = {}
        for step in self.times:
            for layer in self.times[step]:
                for category in self.times[step][layer]:
                    ...
                    category_totals[category] += self.times[step][layer][category]

        data = [(category, total / batch_size) for category, total in category_totals.items()]
        print("Average time per sample across all steps and layers per category:")
        print(tabulate(data, headers=['Category', 'Avg Time (seconds per sample)'], tablefmt='grid'))
