...
from tome import isinstance_str
import torch
...
def wavelet_transform(data):
    """对数据应用二维离散小波变换，并返回各个子带系数。"""
    coeffs = pywt.dwt2(data, 'haar')
    return coeffs  # LL, (LH, HL, HH)

def wavelet_inverse_transform(LL, LH, HL, HH):
    """使用给定的小波系数执行逆变换。"""
    coeffs = LL, (LH, HL, HH)
...
# def combine_latents(latents, latents_sn):
#     """组合两个特征集，通过小波分解和重构过程。
    
#     参数:
#     latents, latents_sn: PyTorch张量，维度为(batch_size, dimension, frame, height, width)，可能在CUDA上。
    
#     返回:
#     新的latents张量，具有与输入相同的形状，保持在原始设备上。
#     """
#     original_device = latents.device

#     # 确保输入是在CPU上
#     latents = latents.cpu().numpy()
#     latents_sn = latents_sn.cpu().numpy()

#     batch_size, dimension, frame, height, width = latents.shape

#     # 新的latent数组
#     new_latents = np.empty_like(latents)
#     print("yes" * 10)
#     # 遍历批次、维度和帧
...
#                 # 小波分解
#                 LL_latents, (LH_latents, HL_latents, HH_latents) = wavelet_transform(latents[b, d, f])
#                 LL_latents_sn, (LH_latents_sn, HL_latents_sn, HH_latents_sn) = wavelet_transform(latents_sn[b, d, f])
                
                
#                 LL = LL_latents_sn
#                 LH = LH_latents_sn
#                 HL = HL_latents_sn
#                 HH = HH_latents_sn  # 从latents_sn取HH
                

#                 # HH = HH_latents_sn  # 从latents_sn取HH
#                 # HH = 0.5 * HH_latents_sn + 0.5 * HH_latents
                
#                 # 小波重构
#                 new_latents[b, d, f] = wavelet_inverse_transform(LL, LH, HL, HH)

#     # 将结果转换回PyTorch张量并放回原始设备
#     return torch.from_numpy(new_latents).to(original_device)
...
class StableDiffusion3Pipeline(DiffusionPipeline, SD3LoraLoaderMixin, FromSingleFileMixin):
    ...
    def __call__(
        ...
        each_time = []
        latents_lst = []
        latents_lst.append(latents[0])
        # latents_lst.append(latents)
        self.transformer._tome_info["sim_dis"] = []
        self.transformer._tome_info["sim_avg"] = []
        save_dis_path = os.path.join("/data1/fanghaipeng/project/sora/tomesd/SD3_attend/xt_dis", '{}.pt'.format(name))
        save_avg_path = os.path.join("/data1/fanghaipeng/project/sora/tomesd/SD3_attend/xt_avg", '{}.pt'.format(name))
        os.makedirs("/data1/fanghaipeng/project/sora/tomesd/SD3_attend/xt_dis", exist_ok=True)
        os.makedirs("/data1/fanghaipeng/project/sora/tomesd/SD3_attend/xt_avg", exist_ok=True)
        ...
        torch.save(self.transformer._tome_info["sim_dis"], save_dis_path)
        torch.save(self.transformer._tome_info["sim_avg"], save_avg_path)
