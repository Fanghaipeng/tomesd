...
def compute_avg_distance_and_similarity(norm_hidden_states, h=None, w=None):
    """
    计算 norm_hidden_states 中 token 之间的平均余弦相似度，以及每个 token 到其最相似 token 的 h + w 距离的平均值。
    可以手动指定 h 和 w，或自动获取。

    参数:
        norm_hidden_states: [1, h*w, dimension] 的张量
        h: 高度（可选）
        w: 宽度（可选）

    返回:
        average_similarity: 平均余弦相似度
        average_distance: 平均 h + w 距离
    """
    # 获取设备
    device = norm_hidden_states.device

    # 获取维度信息
    batch_size, num_tokens, dimension = norm_hidden_states.shape

    if h is None or w is None:
        # 尝试推断 h 和 w
        h = w = int(num_tokens ** 0.5)
        if h * w != num_tokens:
            factors = [(i, num_tokens // i) for i in range(1, num_tokens + 1) if num_tokens % i == 0]
            h, w = factors[len(factors) // 2]  # 选择中间的因子组合
            print(f"自动推断的 h: {h}, w: {w}")
    else:
        if h * w != num_tokens:
            raise ValueError("提供的 h 和 w 与 num_tokens 不匹配。")

    # import ipdb
    # ipdb.set_trace()
    # 提取 tokens，形状为 [h*w, dimension]
    tokens = norm_hidden_states[0]

    # 对 tokens 进行归一化，以确保余弦相似度的计算准确
    tokens_norm = tokens / tokens.norm(dim=1, keepdim=True)

    # 计算余弦相似度矩阵，形状为 [h*w, h*w]
    cosine_sim = tokens_norm @ tokens_norm.T

    # 将对角线元素（自身相似度）设置为 -1，以便在寻找最大值时忽略自身
    cosine_sim.fill_diagonal_(-1)
    cosine_sim = cosine_sim.to(torch.float32)  # 转换数据类型

    # 对于每个 token，找到最相似的 token 的索引
    most_similar_indices = torch.argmax(cosine_sim, dim=1)

    # 计算平均余弦相似度，排除自身相似度
    cosine_sim.fill_diagonal_(0)  # 将对角线元素设回 0
    # total_similarity = cosine_sim.sum()
    # num_pairs = num_tokens * (num_tokens)  # 排除自身
    # average_similarity = total_similarity / num_pairs
    total_similarity = cosine_sim.max(dim=-1).values.sum()
    average_similarity = total_similarity / num_tokens

    # 生成每个 token 的位置坐标，形状为 [h*w, 2]
    positions = torch.stack(torch.meshgrid(torch.arange(h), torch.arange(w), indexing='ij'), dim=-1).reshape(-1, 2)

    # 将 positions 移动到与 norm_hidden_states 相同的设备
    positions = positions.to(device)

    # 确保 most_similar_indices 为 long 类型
    most_similar_indices = most_similar_indices.long()

    # # 计算每个 token 到其最相似 token 的位置差
    # deltas = torch.abs(positions - positions[most_similar_indices])  # [h*w, 2]

    # # 计算 h + w 距离
    # h_plus_w_distances = deltas.sum(dim=1)

    # 计算每个 token 到其最相似 token 的位置差的平方
    deltas_squared = (positions - positions[most_similar_indices]).pow(2)  # [h*w, 2]

    # 计算 L2 距离（欧式距离）
    h_plus_w_distances = deltas_squared.sum(dim=1).sqrt()  # [h*w]

    # 计算平均 h + w 距离
    average_distance = h_plus_w_distances.float().mean()

    return average_similarity.item(), average_distance.item()
...
def make_tome_block_mmdit(block_class: Type[torch.nn.Module]) -> Type[torch.nn.Module]:
    ...
    class JointTransformerBlock_ToMe(block_class):
        ...
        def forward(
            ...
            # NOTE : 统计相似度和距离
            sim_avg, dis_avg = compute_avg_distance_and_similarity(norm_hidden_states)
            self._tome_info["sim_dis"].append(dis_avg)
            self._tome_info["sim_avg"].append(sim_avg)
