...
def calculate_distances(array):
    distances = np.zeros(len(array)) if len(array) > 1 else []
    for i in range(len(array)):
        dis = np.linalg.norm(array[i])
        # dis = np.sum(np.abs(array[i]))
        # dis = np.linalg.norm(array[i], ord=3)
        # dis = np.power(np.sum(np.power(np.abs(array[i]), 3)), 1/3)
        # dis = np.power(np.sum(np.power(np.abs(array[i]), 4)), 1/4)

        distances[i] = dis
        # distances[i] = np.linalg.norm(array[i])
        # distances[i] = math.sqrt(np.linalg.norm(array[i]))
        # distances[i] = math.sqrt(math.sqrt(np.linalg.norm(array[i])))
        # distances[i] = np.log(dis / 4096 + 1e-5)
        # distances[i] = np.log10(np.linalg.norm(array[i]))
        # distances[i] = np.power(np.linalg.norm(array[i]), 0.01)
        # distances[i] = np.power(dis, 0.001)
        # distances[i] = math.sqrt(math.sqrt(math.sqrt(dis)))
        # distances[i] = np.log(dis)
        # distances[i] = np.log2(dis)
...
def plot_distances(dis_LL, dis_LH, dis_HL, dis_HH, path):
    ...
    # 保存图形到文件
    save_path = path.replace('xt', 'l2norm').replace('.pt', '.png')
...
def plot_stats(stats):
    ...
    for idx, comp in enumerate(components):
        ...
        mean_curve = np.round(mean_curve, 1)
        max_curve = np.round(max_curve, 1)
        min_curve = np.round(min_curve, 1)

        if comp == 'LH' or comp == 'HL' or comp == 'HH':

            print(f"{comp} Mean")
            for i in range(len(mean_curve)):
                print(f"{mean_curve[i]}")
            ipdb.set_trace()  
            print(f"{comp} Max")
            for i in range(len(max_curve)):
                print(f"{max_curve[i]}")
            ipdb.set_trace()
            print(f"{comp} Min")
            for i in range(len(min_curve)):
                print(f"{min_curve[i]}")
    ...
    plt.tight_layout(pad=3.0)
    plt.savefig('l2_batch_stats.png')
    plt.close()
