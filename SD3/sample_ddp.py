...
from tome_ours import apply_patch_ToMe_STD, show_timing_info
...
def main(args):
    ...
    output_path = os.path.join(
        args.output_path,
        f"{pipe.__class__.__name__}-{args.height}-{args.width}-{args.num_inference_steps}-{args.guidance_scale}-{args.torch_dtype}-{args.tome_type}-prune{args.prune_replace}-step{args.replace_step}-{args.ratio}-{args.ratio_start}-{args.ratio_end}"
    )
    ...
    if args.tome_type == "default":
        ...
        apply_patch_tome(pipe, ratio=args.ratio, ratio_start=args.ratio_start, ratio_end=args.ratio_end, sx=4, sy=4, \
            save_dir=output_path, prune_replace=args.prune_replace, replace_step=args.replace_step)
